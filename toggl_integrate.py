...
parser.add_argument(
    "-o", "--one", action="store_true", help="Process only one day and exit"
)
parser.add_argument(
    "-p",
    "--projects_only",
...
response = response.json()
projects = [
    {"id": item["id"], "name": item["name"], "active": item["active"], "archive": True}
    for item in response
]

# Find user id
[odoo_user] = models.execute_kw(
    odoo_db,
    uid,
    odoo_password,
    "res.users",
...
if len(ids) > 0:
    ...
    for task in open_tasks:
        ...
        if not found:
            ...
            data = {"project": {"name": task["name"], "wid": wid, "color": "13"}}
...
[work] = models.execute_kw(
    odoo_db,
    uid,
    odoo_password,
    "account.analytic.line",
    "search_read",
    [[["user_id", "=", odoo_user["id"]], ["is_timesheet", "=", True]]],
...
until = datetime.now().replace(hour=0, minute=0, second=0, microsecond=0) + timedelta(
    days=-1
)
...
while (cur_date <= until) and not args.projects_only:
    # Informative message
    print(
        "Processing Toggl time entries from {0}...".format(
    ...
    response = requests.get(
        reports_url, params=params, auth=HTTPBasicAuth(toggl_api_token, "api_token")
    )
    if response.status_code != 200:
    ...
    if len(response["data"]) > 0:
        sys.exit(
            "There are " + str(len(response)) + " entries with no associated project!"
        )
